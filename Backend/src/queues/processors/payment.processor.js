...
async function handleDeliveryCompleted(data) {
    const {
        ...
        storeId, deliveryPartnerId,
        riderEarning = 0, platformProfit = 0,
        commissionAmount = 0,
        total = 0, paymentMethod
    } = data;

    // 1. Credit store wallet with their commission (payout)
    if (storeId && commissionAmount > 0) {
        try {
            await creditWallet({
                entityType: 'store',
                entityId: storeId,
                amount: commissionAmount,
                description: `Order ${orderId} - store commission`,
                category: 'commission',
                orderId: orderMongoId,
                metadata: { orderId, paymentMethod }
            });
            logger.info(`[PaymentProcessor] Store ${storeId} credited ${commissionAmount} for order ${orderId}`);
        } catch (err) {
            logger.error(`[PaymentProcessor] Failed to credit store: ${err.message}`);
        }
    }
