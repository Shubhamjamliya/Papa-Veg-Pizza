...
/**
 * Create a settlement (payout request) for a store or delivery partner.
...
export async function createSettlement({ entityType, entityId, amount, notes = '', periodStart, periodEnd }) {
    if (!['store', 'deliveryBoy'].includes(entityType)) {
        throw new Error('Settlements only for store or deliveryBoy');
    }
...
function resolveWalletForSettlement(entityType, entityId) {
    ...
    if (entityType === 'store') {
        ...
        return {
            Model: mongoose.model('FoodStoreWallet'),
            filter: { storeId: id }
        };
    }
