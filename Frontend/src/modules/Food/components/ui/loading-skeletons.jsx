...
function StoreCardSkeleton({ className, compact = false }) {
  return (
...
function StoreGridSkeleton({ count = DEFAULT_CARD_COUNT, className, compact = false }) {
  return (
    <div className={cn("grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4", className)}>
      {Array.from({ length: count }, (_, index) => (
        <StoreCardSkeleton key={`store-card-${index}`} compact={compact} />
      ))}
    </div>
  )
}
...
function StoreDetailSkeleton({ className }) {
  return (
    <LoadingSkeletonRegion label="Loading store details" className={cn("min-h-screen bg-white dark:bg-[#0a0a0a]", className)}>
