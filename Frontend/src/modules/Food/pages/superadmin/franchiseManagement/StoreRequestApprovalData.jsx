  ...
  return (
    <div className="xl:col-span-3 bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden shadow-sm flex flex-col">
        ...
        <table className="w-full text-left border-collapse min-w-[800px]">
          <thead>
            <tr className="bg-zinc-50 dark:bg-zinc-900/50 border-b border-zinc-200 dark:border-zinc-800">
              <th className="px-6 py-4 text-xs font-bold text-zinc-500 uppercase tracking-wider w-12 text-center">
                <input
                  type="checkbox"
                  className="w-4 h-4 rounded border-zinc-350 dark:border-zinc-700 text-[var(--primary)] focus:ring-[var(--primary)] cursor-pointer"
          ...
          <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
            {filteredRequests.map((req, idx) => {
              ...
              return (
                <tr
                  key={idx}
                  className={`hover:bg-zinc-50 dark:hover:bg-zinc-850/50 transition-colors group cursor-pointer ${
                    isSelected ? "bg-zinc-50/80 dark:bg-zinc-900/50" : ""
                  }`}
                ...
                >
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-1">
                      <button
                        className="p-2 hover:bg-zinc-200 dark:hover:bg-zinc-700 rounded text-zinc-500 transition-colors"
                        title="Review"
                        onClick={(e) => { e.stopPropagation(); onRowClick && onRowClick(req); }}
                      >
                        <Eye size={18} />
                      </button>
                      <button className="p-2 hover:bg-[var(--primary)]/10 rounded text-[var(--primary)] transition-colors" title="Approve" onClick={(e) => e.stopPropagation()}>
                        <CheckCircle size={18} />
                      </button>
                      <button className="p-2 hover:bg-red-100 dark:hover:bg-red-900/30 rounded text-red-500 transition-colors" title="Reject" onClick={(e) => e.stopPropagation()}>
                        <XCircle size={18} />
                      </button>
                    </div>
                  </td>
      ...
      <div className="p-4 md:p-6 bg-white dark:bg-zinc-950 border-t border-zinc-200 dark:border-zinc-800 flex flex-col sm:flex-row items-center justify-between gap-4 mt-auto">
        <p className="text-xs text-zinc-500">Showing 1 to {filteredRequests.length} of {activeRequests.length} entries</p>
