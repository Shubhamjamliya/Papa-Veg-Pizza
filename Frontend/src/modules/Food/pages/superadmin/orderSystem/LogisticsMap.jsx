  ...
  return (
    <div className="fixed inset-0 z-[80] bg-surface dark:bg-zinc-950 flex flex-col animate-in fade-in zoom-in-95 duration-300">
      ...
      <div className="absolute top-4 right-4 z-50">
        <button 
          onClick={onClose}
          className="w-10 h-10 flex items-center justify-center rounded-full bg-white dark:bg-zinc-800 shadow-md border border-zinc-200 dark:border-zinc-700 hover:bg-zinc-100 dark:hover:bg-zinc-700 transition-colors text-zinc-700 dark:text-zinc-300 active:scale-95"
        >
          <X size={20} />
        </button>
      </div>
      ...
      <main className="flex-grow relative overflow-hidden bg-[#e5e5f7]">
        ...
        {/* Floating Search Bar */}
        <div className="absolute top-6 left-1/2 -translate-x-1/2 w-full max-w-md px-6 z-10">
          <div className="bg-white dark:bg-zinc-900 shadow-lg border border-zinc-200 dark:border-zinc-800 rounded-full h-12 flex items-center px-4 gap-3">
            <Search className="text-zinc-400" size={20} />
            <input 
              className="flex-grow bg-transparent border-none focus:ring-0 text-sm text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-400 outline-none" 
        ...
        {/* Floating Filters */}
        <div className="absolute bottom-48 md:bottom-32 left-1/2 -translate-x-1/2 z-10 flex flex-wrap justify-center gap-2 px-6 w-full max-w-3xl">
          <button className="bg-[var(--primary)] text-white px-4 py-2 rounded-full shadow-md text-xs font-bold flex items-center gap-2 transition-all hover:scale-105 active:scale-95">
            <Store size={16} />
            Stores
          </button>
          <button className="bg-white dark:bg-zinc-900 text-zinc-700 dark:text-zinc-300 border border-zinc-200 dark:border-zinc-800 px-4 py-2 rounded-full shadow-md text-xs font-bold flex items-center gap-2 transition-all hover:bg-zinc-50 dark:hover:bg-zinc-800">
            <CheckCircle2 size={16} className="text-emerald-500" />
            Active Deliveries
          </button>
          <button className="bg-white dark:bg-zinc-900 text-zinc-700 dark:text-zinc-300 border border-zinc-200 dark:border-zinc-800 px-4 py-2 rounded-full shadow-md text-xs font-bold flex items-center gap-2 transition-all hover:bg-zinc-50 dark:hover:bg-zinc-800">
            <PauseCircle size={16} className="text-zinc-400" />
            Idle Riders
          </button>
          <button className="bg-white dark:bg-zinc-900 text-red-600 dark:text-red-400 border border-zinc-200 dark:border-zinc-800 px-4 py-2 rounded-full shadow-md text-xs font-bold flex items-center gap-2 transition-all hover:bg-red-50 dark:hover:bg-red-900/20">
            <AlertCircle size={16} />
            Critical Delays
          </button>
        </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 z-20">
          <div className="bg-white dark:bg-zinc-900 shadow-[0_-8px_30px_rgb(0,0,0,0.12)] border-t border-zinc-200 dark:border-zinc-800 rounded-t-3xl p-6 pb-8">
            <div className="max-w-6xl mx-auto">
              <div className="flex flex-col md:flex-row gap-4">
                
                {/* KPI 1: Riders */}
                <div className="flex-1 bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-800 p-4 rounded-2xl flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Active Riders</p>
                    <div className="flex items-baseline gap-2 mt-1">
                      <h2 className="text-2xl font-black text-zinc-900 dark:text-zinc-100">142</h2>
                ...
                <div className="flex-1 bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-800 p-4 rounded-2xl flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Avg. Time</p>
                    <div className="flex items-baseline gap-2 mt-1">
                      <h2 className="text-2xl font-black text-zinc-900 dark:text-zinc-100">22m 14s</h2>
                      <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400 flex items-center">
                        <ArrowDown size={12} className="mr-0.5" /> 3m
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex-1 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-900/30 p-4 rounded-2xl flex items-center gap-4 group cursor-pointer hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors">
                  <div className="w-12 h-12 rounded-xl bg-red-500 flex items-center justify-center text-white shrink-0">
                    <Bell size={24} />
                  </div>
                  <div className="flex-grow">
                    <p className="text-[10px] font-bold text-red-600 dark:text-red-400 uppercase tracking-wider">Critical Delays</p>
                    <div className="flex justify-between items-center mt-1">
                      <h2 className="text-2xl font-black text-red-600 dark:text-red-400">08</h2>
                      <ChevronRight size={20} className="text-red-600 dark:text-red-400 group-hover:translate-x-1 transition-transform" />
                    </div>
                    <p className="text-[10px] text-red-600/80 dark:text-red-400/80 mt-1">Orders exceeding 30 mins limit</p>
                  </div>
                </div>
