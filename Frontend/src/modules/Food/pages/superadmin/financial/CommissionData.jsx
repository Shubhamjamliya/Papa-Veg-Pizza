...
import { Search, Store, MapPin, Filter, Calendar, ChevronDown, MoreVertical } from 'lucide-react';
...
export default function CommissionData({ onViewCommission }) {
  ...
  return (
    <div className="space-y-6">
      {/* Sticky Filter Bar */}
      <nav className="sticky top-[72px] z-30 bg-zinc-50/95 dark:bg-zinc-950/95 backdrop-blur-md border border-zinc-200 dark:border-zinc-800 rounded-xl py-3 px-4 flex flex-wrap items-center gap-4 shadow-sm">
        ...
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input 
            ...
            className="w-full pl-9 pr-4 py-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg text-sm focus:border-[var(--primary)] focus:ring-[var(--primary)] outline-none transition-all dark:text-zinc-100"
          />
        </div>
        ...
        <div className="flex items-center gap-3 overflow-x-auto hide-scrollbar pb-1 sm:pb-0">
          <select 
            ...
            className="flex items-center gap-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 px-3 py-2 rounded-lg cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors text-sm font-bold text-zinc-700 dark:text-zinc-300 outline-none appearance-none pr-8 relative"
          ...
          <div className="flex items-center gap-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 px-3 py-2 rounded-lg shrink-0 cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors text-zinc-700 dark:text-zinc-300">
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
          <h3 className="font-bold text-lg text-zinc-900 dark:text-zinc-100">Recent Commissions</h3>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-zinc-50 dark:bg-zinc-950/50 border-b border-zinc-200 dark:border-zinc-800">
                <th className="px-6 py-4 text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Order / Store</th>
                <th className="px-6 py-4 text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider text-right">Comm %</th>
                <th className="px-6 py-4 text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider text-right">Amount</th>
                ...
                filteredData.map((item, i) => (
                  ...
                  >
                    <td className="px-6 py-4">
                      <p className="font-mono text-sm font-medium text-zinc-900 dark:text-zinc-100 group-hover:text-[var(--primary)] transition-colors">{item.id}</p>
                      <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">{item.store}</p>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <span className="font-mono text-sm font-bold text-zinc-700 dark:text-zinc-300 bg-zinc-100 dark:bg-zinc-800 px-2 py-1 rounded">{item.commPercent}</span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <span className="font-mono text-sm font-bold text-zinc-900 dark:text-zinc-100">{formatCurrency(item.amount)}</span>
                    </td>
                    <td className="px-6 py-4">
