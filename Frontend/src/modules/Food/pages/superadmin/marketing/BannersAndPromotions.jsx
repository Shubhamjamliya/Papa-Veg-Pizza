...
export default function BannersAndPromotions() {
  ...
  if (viewMode === 'calendar') {
    return <BannerCalendar onBack={() => setViewMode('list')} />;
  }
      ...
      <section className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 border-b border-zinc-200 dark:border-zinc-800 pb-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button 
            ...
            className="flex items-center justify-center gap-1.5 px-3 py-1.5 border border-zinc-300 dark:border-zinc-700 text-black/70 dark:text-white/70 rounded-lg text-xs font-bold hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-all shadow-sm"
          >
      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-3.5 shadow-sm">
        ...
        <div className="h-36 w-full relative flex items-end gap-1.5 px-2">
          ...
          {[40, 55, 70, 45, 85, 95, 65, 40, 75, 50, 30, 60, 90, 45, 70, 80, 55, 40, 65, 95].map((height, i) => (
            <div
              key={i}
              className={`flex-1 transition-colors rounded-t-sm relative group cursor-pointer ${i === 5 ? 'bg-[var(--primary)]/30 dark:bg-[var(--primary)]/40 border-x border-t border-[var(--primary)]/50' : 'bg-[var(--primary)]/10 hover:bg-[var(--primary)]/20 dark:bg-[var(--primary)]/20 dark:hover:bg-[var(--primary)]/30'
                }`}
              style={{ height: `${height}%` }}
            >
